...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '62780b34c1891a890e06e776fc12af71308dff95';
...
function mtSignIn() {
    ...
    if (is_preview) {
        if ( document['comments_form'] ) {
            ...
            url += '&return_url=http%3A%2F%2Fblog.youre.space%2Fimyaman%2F';
        }
    } else {
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            phrase = 'このブログにCommentsする権限を持っていません。(\<a href=\"javas\cript:void(0);\" onclick=\"return mtSignOutOnClick();\"\>サインアウトする\<\/a\>)';
        } else {
            var user_link;
            if ( u.is_author ) {
        ...
        if (reg_reqd) {
            phrase = 'Commentsするにはまず\<a href=\"javas\cript:void(0)\" onclick=\"return mtSignInOnClick(\'comment-greeting\')\"\>サインイン\<\/a\>してください。';
        } else {
            phrase = 'Commentsする前に\<a href=\"javas\cript:void(0)\" onclick=\"return mtSignInOnClick(\'comment-greeting\')\"\>サインイン\<\/a\>することもできます。';
        }
    }
    el.innerHTML = phrase;
